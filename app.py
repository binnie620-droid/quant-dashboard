...
with tab1:
    ...
    if today_picks:
        st.dataframe(pd.DataFrame(today_picks).sort_values('sort', ascending=False).drop(columns=['sort']), use_container_width=True, hide_index=True)
    else:
        st.info("😴 현재 확신도 55%를 넘는 종목이 없습니다. 현금을 보유하며 관망하세요.")

with tab2:
    st.subheader("🕵️‍♂️ AI 추천 이후 실제 수익률 추적")
    st.write("지난 10거래일간 AI가 55% 이상의 확신으로 추천했던 종목들의 실적입니다.")
    
    if st.button('매트릭스 정산 시작'):
        # 뱅글뱅글 돌아가는 상태창 추가
        with st.status("과거 데이터를 전수 조사 중입니다...", expanded=True) as status:
            matrix_rows = []
            confidence_threshold = 0.55 # 확신도 유지
            ...
            for name, df in stock_dfs.items():
                ...
                for i in range(20, 10, -1):
                    ...
                    if proba >= confidence_threshold:
                        ...
                        res = {'추천일': past_row.index[0].strftime('%m/%d'), '종목명': name}
                        for d in range(1, 11):
                            idx = -i + d
                            if idx < 0:
            ...
            status.update(label="데이터 정산 완료!", state="complete", expanded=False)

        if matrix_rows:
            df_m = pd.DataFrame(matrix_rows).sort_values(by='추천일', ascending=False)
            st.dataframe(df_m, use_container_width=True, hide_index=True)
        else:
            # ⭐️ 핵심: 종목이 없을 때 출력될 메시지
            st.warning("⚠️ 최근 10거래일 동안 AI 확신도 55%를 넘긴 추천 종목이 단 하나도 없었습니다. 시장 변동성이 컸거나 횡보장이었을 가능성이 높습니다.")

st.sidebar.markdown("---")
st.sidebar.write("✅ **원칙 매매 가이드**")
st.sidebar.caption("확신도 55% 미만은 통계적으로 승률이 낮아 추천에서 제외됩니다. 무리한 진입보다는 기회를 기다리는 것이 퀀트의 자세입니다.")
